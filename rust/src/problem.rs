...
pub fn get_problem(id: u32) -> Option<Problem> {
    ...
    for p in problems.stat_status_pairs.iter() {
        if p.stat.frontend_question_id == id {
            ...
            let resp: RawProblem = client
                ...
                .json(&Query::question_query(
                    p.stat.question_title_slug.as_ref().unwrap(),
                ))
                .send()
                .unwrap()
                .json()
                .unwrap();
            return Some(Problem {
                title: p.stat.question_title.clone().unwrap(),
                title_slug: p.stat.question_title_slug.clone().unwrap(),
